...
const COPY = {
  ...
  body:     { en: 'Sign in to keep this across devices, get a birthday surprise from Wasi, and unlock a personalized concierge.',
    ...
    <div className="bg-gradient-to-r from-amber-50 to-[#FDF3DC] border border-amber-200/60 rounded-2xl p-4 shadow-sm animate-fade-in relative">
      ...
      <div className="flex items-start gap-3 pr-6">
        <div className="w-9 h-9 rounded-full bg-[#0F6E56]/10 flex items-center justify-center flex-shrink-0">
          <ShieldCheck className="w-4.5 h-4.5 text-[#0F6E56]" />
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2.5">
            <button
              onClick={onSignIn}
              className="text-xs font-semibold text-white bg-[#0F6E56] hover:bg-[#0A5C45] px-3.5 py-1.5 rounded-lg cursor-pointer transition active:scale-95 shadow-sm"
            >
              {t('cta', lang)}
            </button>
            <button
